...
import (
	...
	"github.com/Sannrox/tradepipe/scylla/keyspaces"
...
type Keyspaces struct {
	User         *users.User
	Portfolio    *keyspaces.Portfolios
	Savingsplans *keyspaces.SavingsPlans
	Timelines    *keyspaces.Timeline
}
...
func (s *GRPCServer) CreateKeySpaceConnection(contactPoint string, dbAttempts int, delay time.Duration) error {
	if err := scylla.TryToConnectWithRetry(contactPoint, dbAttempts, delay); err != nil {
		return err
	}

	var err error
	s.User, err = users.NewUserKeyspace(contactPoint, user_keyspace)
	if err != nil {
		return err
	}

	s.Portfolio, err = keyspaces.NewPortfolioKeyspace(contactPoint)
	if err != nil {
		return err
	}

	s.Savingsplans, err = keyspaces.NewSavingsPlanKeyspace(contactPoint)
	if err != nil {
		return err
	}

	s.Timelines, err = keyspaces.NewTimelineKeyspace(contactPoint)
	if err != nil {
		return err
	}
