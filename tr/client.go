...
import (
	...
	trTLS "github.com/Sannrox/tradepipe/tls"
	wsconnector "github.com/Sannrox/tradepipe/websocket"
...
func (api *APIClient) VerifyLogin(verifyCode int) error {
	...
	logrus.Debugf("Verify Login: %s", url)
...
func (api *APIClient) NewWebSocketConnection(dataChan chan Message) error {
	...
	if api.TLSConfig == (&tls.Config{}) {
		api.TLSConfig, err = trTLS.CreateTSLClientConfig()
		if err != nil {
			return err
		}
	}
...
func (api *APIClient) CompactPortfolio() (int, error) {
	return api.Subscribe(map[string]interface{}{"type": "compactPortfolio"})
}
